...
import { MC_FORMAT_CODE_COLORS, MC_FORMAT_CODE_STYLES, parseMcFormatCodes, renderMcFormatCodes, stripMcFormatCodes } from '../../src/utils/formatCodes'
...
async function copyColor(color) {
  copiedColor.value = color.foreground

  if (typeof navigator !== 'undefined' && navigator.clipboard) {
    await navigator.clipboard.writeText(color.foreground)
  }

  window.setTimeout(() => {
    if (copiedColor.value === color.foreground) {
      copiedColor.value = ''
    }
  }, 1400)
}
...
<div class="mc-demo mc-demo--column">
  <div class="mc-format-code-grid">
    <button
      v-for="color in colors"
      :key="color.code"
      class="mc-format-code-swatch"
      type="button"
      :style="{ backgroundColor: color.background }"
      :aria-label="`复制 ${color.label} 色号 ${color.foreground}`"
      @click="copyColor(color)"
    >
      <span class="mc-format-code-swatch__code">{{ color.code }}</span>
      <span class="mc-format-code-swatch__name" :style="{ color: color.foreground }">{{ color.label }}</span>
      <span class="mc-format-code-swatch__hex">{{ color.foreground }}</span>
      <span class="mc-format-code-swatch__copy">{{ copiedColor === color.foreground ? '已复制' : '点击复制' }}</span>
    </button>
  </div>
</div>
...
.mc-format-code-swatch:hover,
.mc-format-code-swatch:focus-visible {
  filter: brightness(1.12);
  outline: 2px solid #fff;
  outline-offset: 2px;
}

.mc-format-code-swatch:active {
  transform: translateY(1px);
}

.mc-format-code-swatch__code,
.mc-format-code-swatch__name,
.mc-format-code-swatch__hex,
.mc-format-code-swatch__copy {
